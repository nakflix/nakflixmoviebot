...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001845061982').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users) if auth_users else []
auth_channel = environ.get('AUTH_CHANNEL', '-1001845061982')
...
IMDB_TEMPLATE = environ.get("IMDB_TEMPLATE", '<b>Title: <a href={url}>{title}</a> [{year}]\nAlso Known as {aka}\n#Animation 🤼#Action 🌋 #Adventure 🤣 #Comedy 🎭 #Drama 👨‍👩‍👧‍👧 #Family ✨ #Fantasy 🧟 #Horror 📜 #History 👁️‍🗨️ #Mystery 🔪 #Crime 🌹 #Romance 🎞 #Documentary ⛹️‍♂️ #Sport\nRating⭐: {rating}/ 10  \nGenres: {genres} \nPowered by @NAKFLIXTV</b>')

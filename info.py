...
auth_channel = environ.get('AUTH_CHANNEL', '-1001899642588')
auth_grp = environ.get('-1001654755951')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('-1001654755951')
reqst_channel = environ.get('REQST_CHANNEL_ID')
REQST_CHANNEL = int(reqst_channel) if reqst_channel and id_pattern.search(reqst_channel) else None
SUPPORT_CHAT_ID = -1001654755951
NO_RESULTS_MSG = bool(environ.get("NO_RESULTS_MSG", False))

# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://nakflixbot:alpha3720@nakflixbot.gbt7w.mongodb.net/?retryWrites=true&w=majority")
...
IMDB_TEMPLATE = environ.get("IMDB_TEMPLATE", '<b>Title: <a href={url}>{title}</a> [{year}]\nAlso Known as {aka}\nRating⭐: {rating}/ 10  \nGenres: {genres}\n#Animation 🤼#Action 🌋 #Adventure 🤣 #Comedy 🎭 #Drama 👨‍👩‍👧‍👧 #Family ✨ #Fantasy 🧟 #Horror 📜 #History 👁️‍🗨️ #Mystery 🔪 #Crime 🌹 #Romance 🎞 #Documentary ⛹️‍♂️ #Sport ⚔️ #War 🎸 #Music 🗡 #Thriller 🤖 #Sci_Fi \n#{title}\nPowered by @NAKFLIXTV</b>')
LONG_IMDB_DESCRIPTION = is_enabled(environ.get("LONG_IMDB_DESCRIPTION", "False"), False)
SPELL_CHECK_REPLY = is_enabled(environ.get("SPELL_CHECK_REPLY", "True"), True)
MAX_LIST_ELM = environ.get("MAX_LIST_ELM", None)
INDEX_REQ_CHANNEL = int(environ.get('INDEX_REQ_CHANNEL', LOG_CHANNEL))
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1001755253960')).split()]
